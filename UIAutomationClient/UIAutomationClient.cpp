...
extern "C"
{
	...
	{
		char *pszArray = new char[size];
		return reinterpret_cast<size_t>(pszArray);
	}

	DLL_EXPORT void DeleteCharArray(size_t array)
	{
		char *pszArray = reinterpret_cast<char*>(array);
		if (pszArray)
		{
			delete [] pszArray;
		}
	...
	{
		WCHAR *pszArray = new WCHAR[size];
		return reinterpret_cast<size_t>(pszArray);
	}

	DLL_EXPORT void DeleteWCharArray(size_t array)
	{
		WCHAR *pszArray = reinterpret_cast<WCHAR*>(array);
		if (pszArray)
		{
			delete [] pszArray;
		}
	...
	{
		return GetConsoleTitleW(dest, size);
	}
	...
	DLL_EXPORT int GetParentProcessId(int nProcessId)
	{
		...
		if (hProcess)
		{
			...
			if (0 == nRet)
			{
				return reinterpret_cast<int>(stInfo.Reserved3);
			}
		}
	...
	DLL_EXPORT size_t GetProcessCommandLine(UINT nProcessId)
	{
		...
		if (hThread)
		{
			...
			WCHAR *pszCmdLine = new WCHAR[1024];
			pszCmdLine[0] = 0;
			ReadProcessMemory(hProcess, reinterpret_cast<LPCVOID>(dwExitCode), pszCmdLine, 1024*sizeof(WCHAR), &dwReadedBytes); // param 4, max bytes
			return reinterpret_cast<size_t>(pszCmdLine);
		}
